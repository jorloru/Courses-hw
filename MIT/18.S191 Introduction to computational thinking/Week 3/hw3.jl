...
double_letters = alphabet[diag(sample_freq_matrix) .!= 0.0]
...
# ╔═╡ 3269aca1-85bf-47ca-980b-ff004df98361
row_sum = sum.(eachrow(sample_freq_matrix))

# ╔═╡ e43f7ba5-60d5-4d61-9801-8e8e1a3fe474
col_sum = sum.(eachcol(sample_freq_matrix))

# ╔═╡ cc62929e-f9af-11ea-06b9-439ac08dcb52
row_col_answer = md"""
The sum of each row is the same as the sum of each column, since the matrix is symmetrical (it's the same to say that 'a' comes before 'b' as to say that 'b' comes after 'a').

The sum of each row/column is the overall frequency of a given letter.
"""
...
function ngrams(words, n)
	return [words[i:i+n-1] for i in 1:(length(words)-n+1)]
end
...
function completions_cache(grams)
	...
	cache = Dict();
	gramsize = length(grams[1]);
	
	for i in 1:length(grams)

		ngram = grams[i][1:gramsize-1];
		word  = grams[i][gramsize];
		
		entry = get!(cache, ngram, []);
		
		if isempty(entry)
			cache[ngram] = [word];
		else
			cache[ngram] = [entry word];
		end
