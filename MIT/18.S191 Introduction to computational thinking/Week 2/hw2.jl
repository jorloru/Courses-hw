...
function remove_in_each_row_no_vcat(img, column_numbers)
	...
	for (i, j) in enumerate(column_numbers)
		# Former: img′[i, :] .= vcat(img[i, 1:j-1], img[i, j+1:end])
...
It went from 1029 to 687, a 33% improvement. vcat generates a matrix of the final dimensions and populates it, while what we did doesn't generate a new matrix.
"""
...
It went from 1029 to 3, suggesting this is the most optimal way to do it. Using views we don't need to move the elements from one array to another, we are just passing a reference to the memory location of the original array.

While this approach is faster, it requires caution since everythin we do on img´ will now also affect img.
"""
...
function greedy_seam(energies, starting_pixel::Int)
	...
	for i in 2:N
		j = seam[i-1];
		if j == 1
			seam[i] = argmin(energies[i,j:j+1]);
		elseif j == M
			seam[i] = j-2 + argmin(energies[i,j-1:j]);
		else
			seam[i] = j-2 + argmin(energies[i,j-1:j+1]);
		end
	end
...
function least_energy(energies, i, j)
	...
	if i == N # base case
	   return energies[i,j] , 0
		...
		if j == 1
			results = [least_energy(energies, i+1, j)
					   least_energy(energies, i+1, j+1)];
			best = argmin([results[1][1],results[2][1]]);
		elseif j == M
			results = [least_energy(energies, i+1, j-1)
			...
			results = [least_energy(energies, i+1, j-1)
					   least_energy(energies, i+1, j)
					   least_energy(energies, i+1, j+1)];
			best = argmin([results[1][1],results[2][1],results[3][1]]);
		end

		return energies[i,j] + results[best][1] , j + best - 1 - 1*(j>1)
	end
end
...
The algorithm calculates every possibility in a branching path starting from the first pixel. For each of the possible paths, it calculates every possibility regardless of whether it's been calculated in other path or not.

This algorithm is of order O(n*3^m).
"""
